...
Evaluate this expression in your =*scratch*= buffer to bootstrap =quelpa= now. Add it to your Emacs init file to make sure =quelpa= will be installed automatically if necessary.
...
#+BEGIN_SRC emacs-lisp
(quelpa 'company :upgrade t) ; quelpa will try to upgrade `company' no matter what
(quelpa '(ag :repo "Wilfred/ag.el" :fetcher github) :upgrade nil) ; quelpa will not upgrade `helm'
#+END_SRC
