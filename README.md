...
To setup Emacs on Microsoft Windows so that it work with `quelpa` is a bit tricky so we have added some [Instructions for Windows](#windows). Please consider them before trying to use `quelpa` on Windows.
